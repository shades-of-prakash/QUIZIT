      ...
      if ((document.hidden || !document.fullscreenElement) && sessionLoaded && !submitted) {
        setTabSwitchCount(prev => {
          ...
          if (newCount <= 3) {
            ...
            toast.error("🚨 You exited fullscreen or switched tabs 3 times. Auto-submitting quiz.");
            handleSubmit();
          }
          return newCount;
        });
      ...
      setQuestions(prevQuestions => {
        ...
        if (isMultiple) {
          if (currentOptions.includes(optionIndex)) {
            current.user_options = currentOptions.filter(i => i !== optionIndex);
          } else {
      ...
      <div className="w-screen h-dvh flex flex-col gap-2 overflow-hidden">
        ...
        <div className="w-full bg-white flex justify-between items-center px-4 py-3">
          <div className="flex gap-2 items-center">
            ...
            <div className="text-[10px] p-1 font-semibold bg-neutral-200 rounded-md">RVR&JC</div>
          </div>
          <div className="ml-32">
            <Slider
            ...
            <div className="w-1/2 flex flex-col p-10 gap-3">
              ...
              <div className="flex flex-col gap-6 overflow-auto">
                {questions[activeQuestion]?.options?.map((opt, i) => {
                  const isChecked = questions[activeQuestion]?.user_options?.includes(i) || false;
